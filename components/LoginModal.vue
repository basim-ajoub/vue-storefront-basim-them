    ...
    <transition name="sf-fade" mode="out-in">
      <div v-if="currentScreen === SCREEN_LOGIN">
        <ValidationObserver v-slot="{ handleSubmit }" key="log-in">
          <form class="form" @submit.prevent="handleSubmit(handleLogin)">
            ...
            <SfButton v-e2e="'login-modal-submit'"
              type="submit"
              class="sf-button--full-width form__button login-margin-btn"
              :disabled="loading"
            >
       ...
       <!--  <p class="bottom__paragraph">{{ $t('No account') }}</p>-->
          <SfButton class="sf-button--text  login-reg-btn reg-btn btn-clr " @click="setCurrentScreen(SCREEN_REGISTER)">
            {{ $t('Register today') }}
          </SfButton>
        </div>
      </div>
      ...
      <div v-else class="form">
        ...
        <div class="action login-reg-btn login-btn ">
          <!--{{ $t('or') }}-->
          <SfButton class="sf-button--text btn-clr" @click="setCurrentScreen(SCREEN_LOGIN)">
      {{ $t('Login') }}
          </SfButton>
        </div>
      </div>
...
.modal {
  --modal-index: 3;
  --overlay-z-index: 2;
--input-color:var(--_c-dark-primary);

}
//Start Position login and reg location
.login-reg-btn{
  position: absolute;
  top: calc(var(--spacer-xs) - var(--spacer-xl));
  left: 76%;
}
.login-btn{
  top: calc(var(--spacer-xs) - var(--spacer-2xl));
  left: 78%;

}
//End Position login and reg location

.form {
  margin-top: var(--spacer-xl);
  &__element {
    margin: var(--spacer-xxs) 0 var(--spacer-xl) 0;
  }
  z-index: 5;
&::after{
  content: "";
  position: absolute;
  width: 63%;
  height:calc(var(--spacer-base) + var(--spacer-xl)) ;
  top:  calc(var(--spacer-lg) - var(--spacer-2xl));
  background: var(--_c-dark-secondary);
...
&::before{
  ...
  position: absolute;
  width: 100%;
  height:calc(var(--spacer-2xl) - var(--spacer-base)) ;
  top:  calc(var(--spacer-xs) - var(--spacer-base));
